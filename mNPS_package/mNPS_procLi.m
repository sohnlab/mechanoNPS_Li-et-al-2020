function [ output_matrix ] = mNPS_procLi( filepath, thresholds )
...
%   proceed with user input
    ...
    all_out = mNPS_readLi(data, sampleRate, thresholds, false, false);
    
    %% remove duplicate files to read
        
    [uni_win, output_matrix] = mNPS_cleanLi(all_out);
        ...
        while (searchflag)
            try
                ...
                if emptyflag
                    ...
                    [iter_out, ~, auto] = mNPS_readLi(iterdata, sampleRate, new_th, true, true);
                    searchflag = false;
                end
